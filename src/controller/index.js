...
const getOriginUrlById = async (req, res, ShortId) => {
    try {
        ...
        if(result === null) {
            ...
            if(result.length !== 0) {
                result = result[0]
                redis.set(id, { url: result['url'], expireAt: result['expireAt'] })
            } else {
                redis.set(id, { url: null, expireAt: Date.now() / 1000 })
            }
        }
        //redis有，直接從redis返回
...
const insertOriginUrl = async (req, res) => {
    try {
        ...
        // 插入redis
        redis.set(data['id'], { url: url, expireAt: expireAt })
        // 得到新增的id後
